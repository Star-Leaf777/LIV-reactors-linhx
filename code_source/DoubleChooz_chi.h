...
double chiDCNorm(int exp, int rule, int n_params, double *x, double *errors, void *user_data)
{
  ...
  for (i=ew_low; i <= ew_high; i++)
  {
    ...
    if(k<38) {
      if(fit_rate>bg_rates_FD[i]) {chi2_FD += likelihood(true_rate, fit_rate, true_rate);freedom++;}
      if(fit_rate<=bg_rates_FD[i]) {chi2_FD += 0;}
    }
    k++;
  }
  ...
  for (i=ew_low; i <= ew_high; i++)
  {
    ...
    if(k<76) {
      if(fit_rate>bg_rates_ND[i]) {chi2_ND += likelihood(true_rate, fit_rate, true_rate);freedom++;}
      if(fit_rate<=bg_rates_ND[i]) {chi2_ND += 0;}
    }
    k++;
  }
  ...
  // if(trigger_DC == 0) {ArM_initial(&ArM_params_DC,n);trigger_DC++;}
  ...
  // ArM_select(&ArM_params_DC,&ArM_params_t,n);
  return chi2;
}
